...
/** Target height for the compact kitchen card (fraction of expanded height). */
const KITCHEN_COMPACT_HEIGHT_RATIO = 0.4;

const KITCHEN_COMPACT_DURATION_MS = 1700;

/** Estimated stack below the compact kitchen when all prompt bars are closed. */
const THIRD_SECTION_KITCHEN_BARS_GAP_EM = 0.5;
const THIRD_SECTION_BAR_STACK_GAP_EM = 0.45;
const THIRD_SECTION_CLOSED_BAR_HEIGHT_EM = 3.2;
...
export default function Home() {
  ...
  const kitchenCompactAnimatingRef = useRef(false);
  ...
  useLayoutEffect(() => {
    ...
    const onExpandComplete = () => {
      ...
      kitchenGlideRef.current = {
        settledTop: rect.top,
        finalTop: getKitchenGlideFinalTop(rect.height),
  ...
  const getKitchenCompactHeight = (fullHeight: number) => {
    const rootEm = window.innerWidth / 24.375;
    return Math.max(fullHeight * KITCHEN_COMPACT_HEIGHT_RATIO, rootEm * 11);
  };

  const getThirdSectionStackHeightEstimate = (kitchenHeight: number) => {
    const rootEm = window.innerWidth / 24.375;
    const kitchenBarsGap = rootEm * THIRD_SECTION_KITCHEN_BARS_GAP_EM;
    const barGap = rootEm * THIRD_SECTION_BAR_STACK_GAP_EM;
    const closedBarHeight = rootEm * THIRD_SECTION_CLOSED_BAR_HEIGHT_EM;
    return (
      kitchenHeight +
      kitchenBarsGap +
      THIRD_SECTION_PROMPT_BARS * closedBarHeight +
      (THIRD_SECTION_PROMPT_BARS - 1) * barGap
    );
  };

  const measureThirdSectionStackHeight = (
    sec3: HTMLElement,
    ...
    if (barsHeight > 0) {
      return kitchenHeight + kitchenBarsGap + barsHeight;
    }
    return getThirdSectionStackHeightEstimate(kitchenHeight);
  };

  /** Vertically center the kitchen + bars stack in the band below the nav. */
  const getKitchenViewportTopForCenteredStack = (stackHeight: number) => {
    const vh = getViewportHeight();
    const vvTop = getViewportOffsetTop();
    const nav = document.querySelector(".mobile-nav");
    const navBottom = nav?.getBoundingClientRect().bottom ?? 0;
    const available = Math.max(vh - navBottom, stackHeight);
    return vvTop + navBottom + (available - stackHeight) * 0.5;
  };

  const getKitchenTopForCenteredStack = (
    sec3: HTMLElement,
    stackHeight: number,
  ) => {
    const secRect = sec3.getBoundingClientRect();
    return getKitchenViewportTopForCenteredStack(stackHeight) - secRect.top;
  };

  const getKitchenGlideFinalTop = (fullHeight: number) => {
    const compactHeight = getKitchenCompactHeight(fullHeight);
    const stackHeight = getThirdSectionStackHeightEstimate(compactHeight);
    return getKitchenViewportTopForCenteredStack(stackHeight);
  };
  ...
  useLayoutEffect(() => {
    ...
    const apply = (useLiveRect: boolean) => {
      ...
      const secRect = sec3.getBoundingClientRect();
      const rootEm = window.innerWidth / 24.375;
      const compactHeight = getKitchenCompactHeight(g.height);
      ...
      const stackHeight = measureThirdSectionStackHeight(
        sec3,
        isCompact ? compactHeight : getKitchenCompactHeight(g.height),
      );
      let top = getKitchenTopForCenteredStack(sec3, stackHeight);
      ...
      const vh = getViewportHeight();
      sec3.style.minHeight = `${Math.max(vh, top + stackHeight + rootEm * 2)}px`;
    ...
    const onResize = () => {
      if (kitchenCompactAnimatingRef.current) return;
      ...
      if (kitchenCompactStartedRef.current) {
        const stackHeight = measureThirdSectionStackHeight(sec3, compactHeight);
        const top = getKitchenTopForCenteredStack(sec3, stackHeight);
  ...
  // Re-center the kitchen + bars stack as bar heights change.
  useLayoutEffect(() => {
    if (!kitchenAnchoredInThird || !kitchenCompactStartedRef.current) return;
    if (kitchenCompactAnimatingRef.current) return;

    const sec3 = thirdSectionRef.current;
    const overlay = kitchenOverlayRef.current;
    if (!sec3 || !overlay) return;

    const g = kitchenGlideRef.current;
    const rootEm = window.innerWidth / 24.375;
    const compactHeight =
      kitchenCompactHeightRef.current || getKitchenCompactHeight(g.height);
    const stackHeight = measureThirdSectionStackHeight(sec3, compactHeight);
    const top = getKitchenTopForCenteredStack(sec3, stackHeight);
    const left = Math.max(0, (sec3.clientWidth - g.width) / 2);
    const vh = getViewportHeight();
  ...
  useEffect(() => {
    ...
    syncKitchenSectionVars(sec3, {
      ...
      compactHeight:
        ...
        getKitchenCompactHeight(kitchenGlideRef.current.height),
    });
  }, [
    kitchenAnchoredInThird,
    kitchenCompact,
  ...
  useEffect(() => {
    ...
    const kitchenCompactDurationMs = KITCHEN_COMPACT_DURATION_MS;
    ...
    timers.push(
      setTimeout(() => {
        ...
        const stackHeight = getThirdSectionStackHeightEstimate(compactHeight);
        const centeredTop = getKitchenTopForCenteredStack(sec3, stackHeight);

        kitchenCompactAnimatingRef.current = true;
        setKitchenCompactAnimating(true);

        const left = Math.max(0, (sec3.clientWidth - g.width) / 2);
        setKitchenOverlayTopPx(centeredTop);
        syncKitchenSectionVars(sec3, {
          top: centeredTop,
          left,
          width: g.width,
        ...
        // Top-pinned shrink: height animates down, top stays fixed.
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
        ...
        timers.push(
          setTimeout(() => {
            kitchenCompactAnimatingRef.current = false;
  ...
  return (
    <main style={{ minHeight: "100vh", overflowX: "hidden", maxWidth: "100%" }}>
      ...
      <section
        className="mobile-view home-page"
        style={{
          position: "relative",
      ...
      >
        <style>{`
          ...
          .home-page
            .third-section
            .kitchen-expand-overlay.is-compacting
            .listing-card-media {
            flex: 1.45 1 0;
            transition: flex ${KITCHEN_COMPACT_DURATION_MS}ms cubic-bezier(0.33, 1, 0.68, 1);
          }

          .home-page
            .third-section
            .kitchen-expand-overlay.is-compacting
            .listing-card-image-fade {
            height: 72%;
            opacity: 1;
            transition:
              height ${KITCHEN_COMPACT_DURATION_MS}ms cubic-bezier(0.33, 1, 0.68, 1),
              opacity ${KITCHEN_COMPACT_DURATION_MS}ms cubic-bezier(0.33, 1, 0.68, 1);
          }

          .home-page
            .third-section
            .kitchen-expand-overlay.is-compacting
            .kitchen-expand-price-badge {
            opacity: 0;
          ...
          .home-page
            .third-section
            .kitchen-expand-overlay.is-compacting
            .kitchen-expand-overlay__footer {
            padding: 0.42em 0.9em 0.5em 1em;
            transition: padding ${KITCHEN_COMPACT_DURATION_MS}ms
                ...
                return (
                  <div
                    ...
                    className={`kitchen-expand-overlay${
                      ...
                      kitchenCompactAnimating && !kitchenCompact
                        ? " is-compacting"
                        : ""
        ...
        {thirdSectionAnimationComplete ? (
          <FourthSection
            ...
            scrollReveal
            main2Listings
            className="fourth-section--main2-continuum"
          />
        ) : null}
        {thirdSectionAnimationComplete ? (
          <footer
